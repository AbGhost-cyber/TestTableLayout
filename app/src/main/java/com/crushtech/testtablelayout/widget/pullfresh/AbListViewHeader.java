package com.crushtech.testtablelayout.widget.pullfresh;
...
import com.crushtech.testtablelayout.utils.DisplayUtil;
...
public class AbListViewHeader extends LinearLayout {
	...
	private void initView(Context context) {
		...
		DisplayUtil.setPadding(headerView, 0, 10, 0, 10);
		
		//显示箭头与进度
		FrameLayout headImage =  new FrameLayout(context);
		arrowImageView = new ImageView(context);
		//从包里获取的箭头图片
		arrowImage = BitmapFactory.decodeResource(getResources(), R.drawable.ic_baseline_arrow_drop_down_circle_24);
		...
		layoutParamsWW.width = DisplayUtil.scaleValue(mContext, 25);
		layoutParamsWW.height = DisplayUtil.scaleValue(mContext, 25);
		...
		DisplayUtil.setPadding(headTextLayout,0, 0, 0, 0);
		...
		DisplayUtil.setTextSize(tipsTextview,14);
		DisplayUtil.setTextSize(headerTimeView,14);
		...
		layoutParamsWW3.rightMargin = DisplayUtil.scaleValue(mContext, 10);
...
//	}
	...
	public void setHeaderProgressBarColor(int color){
		if(headerProgressBar != null){
			headerProgressBar.setBarColor(color);
		}
	}
