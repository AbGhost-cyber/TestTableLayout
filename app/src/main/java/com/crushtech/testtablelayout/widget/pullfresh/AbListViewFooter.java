...
public class AbListViewFooter extends LinearLayout {
	...
	private void initView(Context context) {
		...
		footerView.setMinimumHeight(DisplayUtil.scaleValue(mContext, 60));
		footerTextView = new TextView(context);
		footerTextView.setGravity(Gravity.CENTER_VERTICAL);
		setTextColor(Color.rgb(107, 107, 107));
		DisplayUtil.setTextSize(footerTextView,14);
		
		DisplayUtil.setPadding(footerView, 0, 10, 0, 10);
		
		footerProgressBar =   new ProgressWheel(context);
		footerProgressBar.setVisibility(View.GONE);
		footerProgressBar.setBarColor(mContext.getResources().getColor(R.color.ProgressWheel_color));
		footerProgressBar.spin();
		...
		layoutParamsWW.rightMargin = DisplayUtil.scaleValue(mContext, 10);
